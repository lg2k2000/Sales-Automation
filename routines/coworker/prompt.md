...
For each transcript: follow `skills/parse-call.md` end-to-end. That skill handles HPE-internal filter, dedupe via Activity Log query, Deal/Contact resolution, Activity Log row creation, DEFCON Tasks, Deal updates, draft email.

Track per-transcript outcome: processed / skipped (HPE-internal) / skipped (already logged) / unresolved (Needs Routing) / failed.
...
5. If thread last activity was >24h ago AND Deal Stage NOT IN [Closed Won, Closed Lost, On Hold]:
   ...
   - Apply Deal-aligned label via Zapier `Add Label to Email` (direct Gmail label MCPs are disconnected).
...
**Actual:**
- Fireflies: <P> processed via parse-call, <S> skipped (HPE-internal/already-logged), <U> unresolved, <F> failed
